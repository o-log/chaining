...
/**
 * основная точка вход парсера цепочек
 * перед использованием нужно порезать файл лога по дням
 * этот скрипт принимает имя файла лога, реже его на куски по первой букве ИД пользователя
 * - чтобы все сессии одного пользователя попали в один кусок
 * - и при этом файл был не слишком большой для ноды
 * и для каждого куска выполняет парсер цепочек
 */
